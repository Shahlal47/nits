...
function DS_goDirections() {
  ...
  $('#route-details').html(
	...
	DS_directions = loadHistoryDataForAnimation();  
	// draw polylines on map
		for (var i = 1; i < DS_directions.length; i++){
			
			//var polyline = new google.maps.Polyline([
			//								new google.maps.LatLng(DS_directions[i-1].latLng[0],DS_directions[i-1].latLng[1]),
			//								new google.maps.LatLng(DS_directions[i].latLng[0],DS_directions[i].latLng[1])
			//							 ], "#0000ff", 10);
			//DS_map.addOverlay(polyline);
...
function DS_directionsLoaded() {
  ...
  var start = DS_directions[0];
  var end = DS_directions[DS_directions.length-1];
  ...
  DS_placemarks['start'] = DS_geHelpers.createPointPlacemark(
      new google.maps.LatLng(start.latLng[0],
                             start.latLng[1]),
      {description: start.data, standardIcon: 'grn-diamond'});
  
  // create the point placemarks for each step in the driving directions
  for (var i = 1; i < DS_directions.length-1; i++) {
    var step = DS_directions[i];
    
    var placemark = DS_geHelpers.createPointPlacemark(
		new google.maps.LatLng(step.latLng[0],
                             step.latLng[1])
        , {description: step.data, standardIcon: 'red-circle'});
  ...
  DS_placemarks['end'] = DS_geHelpers.createPointPlacemark(
      new google.maps.LatLng(end.latLng[0],
                             end.latLng[1]),
      {description: start.data, standardIcon: 'grn-diamond'});
  ...
  for (var i = 0; i < DS_directions.length; i++)
    lineStringKml +=
        DS_directions[i].latLng[1].toString() + ',' +
        DS_directions[i].latLng[0].toString() +
        ',10\n';
...
function DS_buildPathStepArrays() {
  ...
  DS_path = [];
    ...
    var prevLoc = null;
  for (var i = 0; i < DS_directions.length; i++){
	var loc = new google.maps.LatLng(DS_directions[i].latLng[0],DS_directions[i].latLng[1]);
	//console.debug(i);
    if(i>0)
		distance = DS_geHelpers.distance(loc, prevLoc);
    DS_steps.push({
      loc: loc,
      desc: DS_directions[i].data,//step.getDescriptionHtml(),
      distanceHtml: ""+distance,//step.getDistance().html,
      pathIndex: i
    });  
	prevLoc = loc;
	//if(i>0)
    ...
    case 'reset':
      ...
      DS_simulator = new DDSimulator(DS_ge, DS_path, {
        ...
        on_tick: function() {
          ...
          DS_mapMarker.setPosition(DS_simulator.currentLoc);
      ...
      if (!DS_mapMarker) {
        ...
        //DS_mapMarker.setMap(DS_map);
        
        DS_mapMarker = new google.maps.Marker({
            position: new google.maps.LatLng(DS_simulator.currentLoc[0],DS_simulator.currentLoc[1]),
            map: DS_map
        });
        DS_mapMarker.setIcon(icon);
      }
      
      DS_map.setZoom(13);
      //var latlng = new google.maps.LatLng(deviceStatus.vehicleLat, deviceStatus.vehicleLng);
      DS_mapMarker.setPosition(DS_simulator.currentLoc);
