...
(function($) {
  ...
  $.fn.odometer = function(settings) {
    ...
    return this.each(function(){
      ...
      $this.append('<div style="position:absolute; right: 0px; top: 0px; background-image: url(http://jquery.freebaseapps.com/odometer_shadow); background-repeat: repeat-x; width: ' + width + '; height: ' + height + ';"></div>');
      ...
      function pos(i) {
        return zeroSet - (i * settings.heightNumber);
      }
      
      function relPos(oldDigit,newDigit) {
      ...
      function updatePosition(col, newDigit, oldDigit) {
        if (newDigit != oldDigit) {
          col.stop();
          col.animate({top: relPos(oldDigit,newDigit)},settings.speed, settings.easing).animate({top: pos(newDigit)},0,'linear');
        }
      }
    });
  };
  ...
  $.fn.odometer.defaults = {
    ...
    speed: 500,
    easing: 'swing',
    numbersImage: 'http://jquery.freebaseapps.com/odometer_numbers',
    backgroundImage: 'http://jquery.freebaseapps.com/odometer_background',
    heightNumber: 27,
    widthNumber: 20,
    offsetRight: 0,
    spaceNumbers: 0,
