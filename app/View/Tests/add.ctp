...
<?php 
				...
				$data = $this->Js->get('#TestAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#TestAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		...
		$('#ajax-page-title').html('<i class="icon-user"></i> Test : Add');	   
	});
</script>