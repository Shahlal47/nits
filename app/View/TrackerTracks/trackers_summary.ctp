	...
	<div class="row-fluid">
		<div class="span12">
			<div class="box">
				<header>
					<h5 id="history-title">
						<i class="icon-map-marker"></i> Tracking Summary
					</h5>
					<div class="" style="margin: 5px 0 0 20px; float: left">
						<div class="btn-group">
							<a href="javascript:;" id="btnToggleCluster"
								data-toggle="buttons-checkbox" data-placement="bottom"
								data-original-title="Toggle Cluster" rel="tooltip"
								class="btn btn-metis-6"> <i class="icon-asterisk"></i>
							</a>
						</div>
					</div>
					<div class="pull-right" style="padding: 0 0px; width: 180px;">
						<h5 style="margin: 5px 0; padding: 0;">
							Total Trackers : <span style="font-size: 1.5em; padding: 5px;"
								class="label btn-metis-6" id="totalTrackers">2</span>
						</h5>
					</div>
				</header>
				<div id="live-tracking-map" class="gMap"
					style="width: 74%; margin: 0; padding-left: 0; float: left;"></div>
					
				<!--  <div class="body"
					style="padding: 0; float: left; width: 20%; background-color: white; margin: 0; padding: 0;">
					<div id="live-tracker-info" class="tab-pane active">
						<h4>Tracker Images :</h4>
						<hr>
						<table style="width: 100%;background-color: white;" class="" id="trackereye-list">
							<tbody>
				 ...
				 <div class="body"
					style="padding: 0; float: left; width: 25%; background-color: white; margin-left: 1%;">
					<div id="live-tracker-info" class="tab-pane active">
						<table class="tracker" id="tracker-list" style="width: 100%;background-color: white;">
							<tbody>
	...
	var VTS_TRACKING_SUMMARY = {
			...
			initTimer:function () {		        
				$.timer("live_tracking_timer", function() { 
					VTS_TRACKING_SUMMARY.getTrackerStatusData();
				}, VTS_TRACKING_SUMMARY.timer_interval);	
			  ...
			  initDeviceInfo:function(dvPrf){
				...
				setMapCenter:function(lat,lng){
					$(VTS_TRACKING_SUMMARY.mapid).gmap3('get').setCenter(new google.maps.LatLng(lat,lng));
				},
				focusMarkerById : function (id) {
					var clusterer = $(VTS_TRACKING_SUMMARY.mapid).gmap3({
						get : {
							name : "clusterer"
						}
					});
				...
				getTrackerStatusData:function(){
					...
					MAP_HELPER._clusterHack(VTS_TRACKING_SUMMARY.mapid);
					
					var p_data = APP_COMMON.getDeviceStatus();
					APP_COMMON.loadUserDeviceUpdatesByDeviceids(VTS_TRACKING_SUMMARY.deviceids, function(data){
						...
						$.each(p_data, function(i) {						
							...
							if(n_data.recordEventNumber != p_data[i].recordEventNumber){
								//VTS_TRACKING_SUMMARY.changeDeviceStatus(VTS_TRACKING_SUMMARY.mapid, this);	
							}				
							VTS_TRACKING_SUMMARY.changeDeviceStatus(VTS_TRACKING_SUMMARY.mapid, this);									
						});
					});										
				...
				changeDeviceStatus : function (dMap, v) {
					...
					$("#img-" + v.deviceid).attr("src", APP_COMMON.app_root+"img/nits_marker/"+ticon);

					VTS_TRACKING_SUMMARY.changeMarkerInCluster(VTS_TRACKING_SUMMARY.mapid, v);
				},
				changeMarkerInCluster : function (dMap, v) {
					...
					if (clusterer) {
						...
						if (marker) {
							...
							VTS_TRACKING_SUMMARY.createNewMarkerAddInCluster(dMap, v, clusterer);
						}
					} else {
				...
				createTrackerList : function(devices){
					...
					/*	var li = 	'<li id="'+devices[i].ClientDevice.deviceid+'">' +
									...
									'</li>';*/
						var tr = '<tr>' +
							'<td style="width:10%;"><img alt="" src="'+icn+'" class="tracker-btn" id="'+devices[i].ClientDevice.deviceid+'"></td>'+
							'<td style="width:80%;"><div id="'+devices[i].ClientDevice.deviceid+'"><a href="javascript:;" class="lnkTracker"><span><b>'+devices[i].ClientDevice.name+'</b></span></a></div></td>'+
							'<td style="width:10%;"><a href="#" class="tracker-btn" id="'+devices[i].ClientDevice.deviceid+'"><i class="icon-external-link"></i></a></td>'+
							'</tr>';			
						$('#tracker-list').append(tr);		

						VTS_TRACKING_SUMMARY.deviceids.push('"'+devices[i].ClientDevice.deviceid+'"');							
					});
					//console.debug(VTS_TRACKING_SUMMARY.deviceids);
					$('#totalTrackers').html(devices.length);
				},
				loadClusterDataOnMap : function (dMap) {
					var dt = new Array();
					var v = APP_COMMON.getDeviceStatus();
					$.each(v, function(i) {

						var deviceInfo = APP_COMMON.getDeviceProfileByDeviceid(v[i].deviceid);
					...
					MAP_HELPER.initClusterMap(VTS_TRACKING_SUMMARY.mapid, dt);

					google.maps.event.addListenerOnce(map, 'idle', function(){
					    // do something only the first time the map is loaded
					    //MAP_HELPER._clusterHack(VTS_TRACKING_SUMMARY.mapid);
						VTS_TRACKING_SUMMARY.initTimer();
				...
				init: function(){
					...
					VTS_TRACKING_SUMMARY.createTrackerList(devices);

					APP_COMMON.loadUserDeviceUpdatesByDeviceids(VTS_TRACKING_SUMMARY.deviceids, function(data){

						VTS_TRACKING_SUMMARY.loadClusterDataOnMap(VTS_TRACKING_SUMMARY.mapid);
						
						
					});					
					// 
					$('#btnToggleCluster').bind('click',function(){
						MAP_HELPER.toggleCluster(VTS_TRACKING_SUMMARY.mapid, this);
					});
					$('#tracker-list .lnkTracker').bind('click',function(e){
						var deviceid = $(this).parent().attr('id');
						VTS_TRACKING_SUMMARY.focusMarkerById(deviceid);
						e.preventDefault();
					});
					$('#tracker-list .tracker-btn').bind('click',function(e){
						var deviceid = $(this).attr('id');
						VTS_TRACKING_SUMMARY.openTrackerNewTab(deviceid);
						e.preventDefault();
					});
					...
					APP_COMMON.bindResizeDiv(VTS_TRACKING_SUMMARY.mapid,110);
					...
					//VTS_TRACKING_SUMMARY.initSensorUI();
					/*
					VTS_TRACKING_SUMMARY.initGauges();
					VTS_TRACKING_SUMMARY.initOdometer();
