	...
	var VTS_HISTORY = {
		...
		updateHistoryData : function(data) {
			...
			$( dt ).each(function( index ) {
				...
				VTS_HISTORY.markerArrays.push(markerA);
		...
		clearMarkers : function() {
			$.each(VTS_HISTORY.markerArrays, function(index, val) {
			    val.setMap(null);
			});
			for (i=0; i<VTS_HISTORY.line.length; i++) 
			{                           
				VTS_HISTORY.line[i].setMap(null); 
			}
