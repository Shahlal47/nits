...
<script type="text/javascript">
	var FN_AllDevices = {
		...
		AllDeviceListUrl : APP_HELPER.getFullPath("clientDevices/get_all_device_list_json"),
		addDatatableActions : function(oObj) {  
			var id = oObj.aData[0];
			var trackerId = oObj.aData[2];
			 
			//var ondelete = "APP_HELPER.ajaxDeleteRecord('clientDevices/delete/"+id+"?clientid=<?php echo $id?>','<?php echo $ajaxPlaceholder?>')";
			var onTransfer = "APP_HELPER.ajaxRequestModelAction('clientDevices/transferDeviceToSingleUser/" + trackerId +"');";
			var deviceid = oObj.aData[4]; 
// 			return '<a class="btn btn-small btn-warning" title="Transfer" href="javascript:;" onclick="'+onTransfer+'"><i class="btn-icon-only icon-edit"></i></a><a href="#" class="tracker-btn btn btn-small" id="'+deviceid+'" onclick="FN_AllDevices.openTrackerNewTab('+deviceid+')"><i class="icon-external-link"></i></a>';
			return '<a class="btn btn-small btn-warning" title="Transfer" href="javascript:;" onclick="'+onTransfer+'"><i class="btn-icon-only icon-edit"></i></a>';
		},
		...
		initTable : function(){
			...
			$(FN_AllDevices.tableName).dataTable({
	    		...
	    		"aoColumns": [     
							...
							{ 'sName': 'name',  'sWidth': 30, 'sClass': 'center', "bSearchable": true},
							{ 'sName': 'tracker_id',  'sWidth': 30, 'sClass': 'center', },
							{ 'sName': 'mob_no',  'sWidth': 30, 'sClass': 'center', "bSearchable": true},
							{ 'sName': 'deviceid',  'sWidth': 30, 'sClass': 'center', "bSearchable": true},
