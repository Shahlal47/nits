<div class="span12">
	<div class="box">
		...
		<h5>
		<?php echo __('Edit Report Setting'); ?>
		</h5>
		<div class="toolbar">
			<ul class="nav">
				...
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecord('reportSettings/delete/'+$('#ReportSettingId').val());">
		...
		$data = $this->Js->get('#ReportSettingEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
		$this->Js->get('#ReportSettingEditForm')->event(
					'submit',
		$this->Js->request(
		array('action' => 'edit/'.$this->request->data['ReportSetting']['id']),
		array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
...
<script>
	$(function() {
		APP_COMMON.initPage('Edit Report');
		$('#ajax-page-title').html('<i class="icon-user"></i> Report Setting : Edit');	   
	});
</script>
