...
<?php 
				...
				$data = $this->Js->get('#UserTypeAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#UserTypeAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		APP_COMMON.initPage('Add User Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> User Type : Add');	   
	});
</script>