<div class="span12">
	<div class="box dark">
		...
		<h5>
		<?php echo __('Edit Tracker Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxRequestModelAction('trackerTypes/index');"> <i
						class="icon-list-alt"></i> List </a></li>
										<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecord('trackerTypes/delete/'+$('#TrackerTypeId').val());">
...
<?php 
				...
				$data = $this->Js->get('#TrackerTypeEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#TrackerTypeEditForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'edit/'.$this->request->data['TrackerType']['id']),		
				array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
						...
						echo $this->Form->create('TrackerType', array('inputDefaults' => array('label' => false,'div' => false, 'error' => false), 'class'=>'form-horizontal'));
						?>
			<fieldset>
...
<script>
	$(function() {
		...
		$('#ajax-page-title').html('<i class="icon-user"></i> Tracker Type : Edit');	   
	});
</script>