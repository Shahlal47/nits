...
<?php 
				...
				$data = $this->Js->get('#GeofenceAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#GeofenceAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		...
		$('#ajax-page-title').html('<i class="icon-user"></i> Geofence : Add');	   
	});
</script>