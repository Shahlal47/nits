				...
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxRequestModelAction('poiTypes/index');"> <i
						class="icon-list-alt"></i> List </a></li>
										<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecord('poiTypes/delete/'+$('#PoiTypeId').val());">
...
<?php 
				...
				$data = $this->Js->get('#PoiTypeEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#PoiTypeEditForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'edit/'.$this->request->data['PoiType']['id']),		
				array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
...
<script>
	$(function() {
		APP_COMMON.initPage('Edit POI Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Poi Type : Edit');	   
	});
</script>