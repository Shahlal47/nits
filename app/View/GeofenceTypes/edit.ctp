<div class="span12">
	<div class="box dark">
		...
		<h5>
		<?php echo __('Edit Geofence Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxRequestModelAction('geofenceTypes/index');"> <i
						class="icon-list-alt"></i> List </a></li>
										<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecordAction('geofenceTypes/delete/'+$('#GeofenceTypeId').val());">
...
<?php 
				...
				$data = $this->Js->get('#GeofenceTypeEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#GeofenceTypeEditForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'edit/'.$this->request->data['GeofenceType']['id']),		
				array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
...
<script>
	$(function() {
		APP_COMMON.initPage('Edit Geo-Fence Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Geofence Type : Edit');	   
	});
</script>