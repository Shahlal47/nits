...
<?php 
				...
				$data = $this->Js->get('#FuelTypeAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#FuelTypeAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		APP_COMMON.initPage('Add Fuel Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Fuel Type : Add');	   
	});
</script>