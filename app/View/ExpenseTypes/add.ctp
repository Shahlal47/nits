<div class="span12">
	<div class="box">
		...
		<h5>
		<?php echo __('Add Expense Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
...
<?php 
				...
				$data = $this->Js->get('#ExpenseTypeAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#ExpenseTypeAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		APP_COMMON.initPage('Add Expense Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Expense Type : Add');	   
	});
</script>