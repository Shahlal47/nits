<div class="span12">
	<div class="box">
		...
		<h5>
		<?php echo __('Edit Client Contact Device'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxRequestModelAction('clientContactDevices/index');"> <i
						class="icon-list-alt"></i> List </a></li>
										<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecord('clientContactDevices/delete/'+$('#ClientContactDeviceId').val());">
...
<?php 
				...
				$data = $this->Js->get('#ClientContactDeviceEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#ClientContactDeviceEditForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'edit/'.$this->request->data['ClientContactDevice']['id']),		
				array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
...
<script>
	$(function() {
		APP_COMMON.initPage('Edit Device Contacts');
		$('#ajax-page-title').html('<i class="icon-user"></i> Client Contact Device : Edit');	   
	});
</script>