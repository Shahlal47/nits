<div class="span12">
	<div class="box dark">
		...
		<h5>
		<?php echo __('Edit Alert Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
				<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxRequestModelAction('alertTypes/index');"> <i
						class="icon-list-alt"></i> List </a></li>
										<li><a href="javascript:;"
					onclick="APP_HELPER.ajaxDeleteRecordAction('alertTypes/delete/'+$('#AlertTypeId').val());">
...
<?php 
				...
				$data = $this->Js->get('#AlertTypeEditForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#AlertTypeEditForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'edit/'.$this->request->data['AlertType']['id']),		
				array(
						'data' => $data,
						'async' => true,    
						'dataExpression'=>true,
...
<script>
	$(function() {
		...
		APP_COMMON.initPage('Edit Alert-Type');
		
		$('#ajax-page-title').html('<i class="icon-user"></i> Alert Type : Edit');	   
	});
</script>