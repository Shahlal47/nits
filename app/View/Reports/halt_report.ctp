	...
	<div class="box">
		...
		<div class="collapse in body" id="reportFilter">
			...
			<div style="margin: 0%;">
				...
				<?php echo $this->Form->hidden('HaltReport.trackers', array('id'=>'trackers'));?>
				&nbsp;
			</div>
					...
					<tbody>
						...
						<tr>
							<td><span class="add-on"><i class="icon-calendar"> </i> </span> <input
								...
								name="data[HaltReport][todate]"></td>
						</tr>
					</tbody>
				</table>
			</div>
			...
			<div style="margin: 0%;">
				...
				<table id="" style="width: 100%;" class="report_class">
					<tbody>
						<tr>
							<td><input type="radio" name="data[HaltReport][halt]"
								checked="checked" value="0"></input>
							</td>
							<td>All</td>
						</tr>
						<tr>
							<td><input type="radio" name="data[HaltReport][halt]" value="10"></input>
							</td>
							<td>Minimum Halt 10 Min</td>
						</tr>
						<tr>
							<td><input type="radio" name="data[HaltReport][halt]" value="30"></input>
							</td>
							<td>Minimum Halt 30 Min</td>
						</tr>
						<tr>
							<td><input type="radio" name="data[HaltReport][halt]" value="60"></input>
							</td>
							<td>Minimum Halt 60 Min</td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	var ALERT_REPORT = {
			init : function(){
				...
				$('#btnShowReport').bind('click',function(){
					...
					var url = 'reports/ajaxHaltReport';
					var data = $('#CreateHaltReportForm').serialize();
								...
								$.each(val,function(index,r) {
									...
									st += ('<td>'+r.info+'</td>');
								  	...
								  	if(l == index){
										...
										st += ('<td colspan="8" style="text-align:center;"><b>'+r.total_halt+'</b></td>');
									  	st += '<tr>';
									  	}
								});
							});
				...
				$('#btnShowGraph').bind('click',function(){
					...
					var url = 'reports/haltReportGraph';
					var data = $('#CreateHaltReportForm').serialize();
					APP_HELPER.ajaxSubmitDataCallback(url,data,function(d){
						$('#graphdiv').html(d);
					});
