<div class="span12">
	<div class="box dark">
		...
		<h5>
		<?php echo __('Add Company Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
...
<?php 
				...
				$data = $this->Js->get('#CompanyTypeAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#CompanyTypeAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		APP_COMMON.initPage('Add Company Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Company Type : Add');	   
	});
</script>