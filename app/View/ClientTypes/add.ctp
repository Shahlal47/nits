<div class="span12">
	<div class="box dark">
		...
		<h5>
		<?php echo __('Add Client Type'); ?>		</h5>
		<div class="toolbar">
			<ul class="nav">
...
<?php 
				...
				$data = $this->Js->get('#ClientTypeAddForm')->serializeForm(array('isForm' => true, 'inline' => true));
				$this->Js->get('#ClientTypeAddForm')->event(
					'submit',
				$this->Js->request( 		
 array('action' => 'add'),		
				array(
...
<script>
	$(function() {
		APP_COMMON.initPage('Add Client Type');
		$('#ajax-page-title').html('<i class="icon-user"></i> Client Type : Add');	   
	});
</script>